...
def groupSum(start, nums, target):
  if start >= len(nums):
    return target == 0
  if groupSum(start + 1, nums, target - nums[start]) or groupSum(start + 1, nums, target):
    return True
  return False
...
def groupSum6(start, nums, target):
  if start >= len(nums):
    return target == 0
  if groupSum6(start + 1, nums, target - nums[start]) or (nums[start] != 6 and groupSum6(start + 1, nums, target)):
    return True
  return False
...
def groupNoAdj(start, nums, target):
  if start >= len(nums):
    return target == 0
  if groupNoAdj(start + 1, nums, target) or groupNoAdj(start + 2, nums, target - nums[start]):
    return True
  return False
...
def groupSum5(start, nums, target):
  ...
  if groupSum5(start + 1, nums, target - nums[start]):
    if start == 0 or not (start > 0 and nums[start - 1] % 5 == 0 and nums[start] == 1):
      return True
  if nums[start] % 5 != 0 and groupSum5(start + 1, nums, target):
    return True
  return False
...
def splitArrayHelper(initial, nums, first, second):
    if(initial >= (len(nums))):
        return first == second
            
    elif(splitArrayHelper(initial + 1, nums, first + nums[initial], second)):
        return True
                      
    elif(splitArrayHelper(initial + 1, nums, first, second + nums[initial])):
        return True
...
def splitOdd10Helper(nums, index, first, second):
    if index == len(nums):
        return first % 10 == 0 and second % 2 == 1
    return splitOdd10Helper(nums, index + 1, first + nums[index], second) or splitOdd10Helper(nums, index + 1, first, second + nums[index])
...
def split53(nums):
  ...
  three = 0
  five = 0
  return split53Helper(nums, initial, three, five); 

def split53Helper(nums, initial, three, five):   
  if initial >= len(nums):
      return three == five
  start = nums[initial]

  if nums[initial] % 5 == 0:
    return split53Helper(nums,initial + 1, three, five + start)
  elif nums[initial] % 3 == 0:
    return split53Helper(nums, initial + 1, three + start, five)
  else:
      if split53Helper(nums, initial + 1, three + start, five):
          return True
      if split53Helper(nums,initial + 1, three, five + start):
          return True
