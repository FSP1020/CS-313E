...
def countHi2(str):
  ...
  if str[len(str) - 2 : len(str)] == 'hi':
    if len(str) > 2 and str[len(str) - 3] != 'x' or len(str) == 2:
      return 1 + countHi2(str[0 : len(str) - 1])
...
def strCopies(str, sub, n):
  if extra_copies(str, sub) == n:
    return True
  return False

def extra_copies(str, sub):
  if len(str) < len(sub):
    return 0
  if str[0 : len(sub)] == sub:
    return 1 + extra_copies(str[1:], sub)
  return extra_copies(str[1:], sub)

# Given a string and a non-empty substring sub, compute recursively the 
# largest substring which starts and ends with sub and return its length.
def strDist(str, sub):
  return len(Dist(str, sub))
...
def Dist(str, sub):
  ...
  if str[0 : len(sub)] == sub:
    if str[len(str) - len(sub) : len(str)] == sub:
      return str
    return Dist(str[0 : len(str) - 1], sub)
  return Dist(str[1:], sub)
